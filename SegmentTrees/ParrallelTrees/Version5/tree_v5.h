...
class ParallelSegmentTree_V5 : public AbstractTree<T, M, P> {
 private:
  struct Query;
 public:
  template<class Iter>
  explicit ParallelSegmentTree_V5(Iter begin, Iter end, P policy)
      : AbstractTree<T, M, P>(std::move(policy)) {
    if (std::distance(begin, end) < kThreadsCount) {
      throw std::runtime_error("too small array");
    }
    size_t chunk_size = std::distance(begin, end) / SegmentsCount;
    ...
    threads_pool_.emplace_back(&ParallelSegmentTree_V5::RunThread,
                               this,
                               0,
                               chunk_size);
    threads_pool_.emplace_back(&ParallelSegmentTree_V5::RunThread,
  ...
  virtual  ~ParallelSegmentTree_V5() {
